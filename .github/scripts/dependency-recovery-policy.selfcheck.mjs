...
function canonicalFixture() {
  ...
  const pull = {
    ...
    head: {
      ref: 'dependabot/npm_and_yarn/toolchain',
...
function runFixture({
  ...
  headSha = 'b'.repeat(40),
  headBranch = 'dependabot/npm_and_yarn/toolchain',
} = {}) {
  return {
    id: 501,
...
function fakeGithub({
  ...
  const api = {
    rest: {
      git: { getRef: async () => ({ data: { object: { sha: baseSha } } }) },
      pulls: {
        get: async () => ({ data: pull }),
...
test('recovery config is bounded and Mobile infrastructure-only', () => {
  assert.deepEqual(validateRecoveryConfig(recoveryConfig), []);
  assert.equal(recoveryConfig.maxRunAttempts, 2);
  assert.deepEqual(recoveryConfig.transientSteps, [
    'Pin npm runtime',
...
test('only the failed-step timestamp window can authorize recovery', () => {
  const candidate = job();
...
test('allowlisted npm setup with precise network evidence is retryable', () => {
  const result = classifyLeafJobFailure(
    job(),
...
test('missing or malformed failed-step timestamps fail closed', () => {
...
test('Appium, compatibility, validation, and security semantics never become transient', () => {
  for (const step of [
    'Qualify framework contracts',
    'Bind TypeScript API surface to Node 22',
    'Run npm run typecheck',
    ...
    const result = classifyLeafJobFailure(
      job({ step }),
      logs({ failed: 'ETIMEDOUT EAI_AGAIN request failed with status code 503' }),
      recoveryConfig,
    );
...
test('scope admits governed dependency files and rejects control plane or unknown files', () => {
  const base = {
...
test('injected-client orchestration requests exactly one canonical transient rerun', async () => {
  ...
  const transientJob = job({ id: 10, step: 'Install committed dependency graph' });
...
test('native rebasing and recovery wiring remain self-tested control plane', () => {
  ...
  for (const protectedPath of [
    ...
    assert.ok(
      governanceConfig.manualReviewPaths.includes(protectedPath),
  ...
  for (const triggerPath of [
    ...
    assert.ok(workflow.includes(triggerPath), `${triggerPath} must trigger governance self-tests`);
  }
